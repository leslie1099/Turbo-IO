...
NSArray<NSString *> *TWReadingLines(NSString *text){if(![text isKindOfClass:NSString.class]||text.length>64*1024*1024)return nil;
 ...
 [text enumerateSubstringsInRange:NSMakeRange(0,text.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString *c,NSRange a,NSRange b,BOOL *stop){
  if(rows.count>=400000){*stop=YES;return;}
 ...
 }];if(rows.count>=400000)return nil;if(line.length)[rows addObject:line];return rows.count?rows:nil;
}
NSData *TWReaderWindow(NSArray<NSString *> *lines,NSString *title,uint32_t token,NSUInteger top,unsigned speed,BOOL automatic){if(!lines.count||lines.count>400000||top>=lines.count||!token||speed<30||speed>480)return nil;
...
static NSDictionary *ZipEntries(NSData *d){const uint8_t *b=d.bytes;NSUInteger n=d.length;if(n<22||n>256*1024*1024)return nil;
 ...
 for(unsigned i=0;i<count;i++){if(at+46>offset+size||wr_u32(b+at)!=0x02014b50)return nil;const uint8_t *c=b+at;unsigned nameLen=U16(c+28),extra=U16(c+30),comment=U16(c+32);if(at+46+nameLen+extra+comment>offset+size||U16(c+8)&1||U16(c+34))return nil;
  ...
  NSUInteger local=wr_u32(c+42),compressed=wr_u32(c+20),plain=wr_u32(c+24);unsigned method=U16(c+10);if(local+30>offset||plain>128*1024*1024||compressed>n||method>8||(method!=0&&method!=8))return nil;
...
static NSData *Extract(NSData *archive,NSDictionary *entry){if(!entry)return nil;NSUInteger plain=[entry[@"plain"]unsignedIntegerValue],length=[entry[@"compressed"]unsignedIntegerValue];if(plain>128*1024*1024)return nil;const uint8_t *p=(const uint8_t *)archive.bytes+[entry[@"at"]unsignedIntegerValue];NSMutableData *d=[NSMutableData dataWithLength:plain];
...
NSString *TWImportBook(NSData *data,NSString *extension,NSString **error){if(error)*error=@"导入失败：仅支持无加密 EPUB 或 UTF-8/UTF-16 TXT，文件不超过256 MiB";if(!data||data.length>256*1024*1024)return nil;
 if([extension.lowercaseString isEqual:@"txt"]){NSString *s=[[NSString alloc]initWithData:data encoding:NSUTF8StringEncoding];if(!s&&data.length>=2){const uint8_t *b=data.bytes;if((b[0]==255&&b[1]==254)||(b[0]==254&&b[1]==255))s=[[NSString alloc]initWithData:data encoding:NSUTF16StringEncoding];}return s.length<=64*1024*1024?s:nil;}
